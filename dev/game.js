...
function make_shan(rule, log) {
  ...
  for (let i = 0; i < shan._pai.length; i++) {
    shan._pai[i] = "_";
  }
  ...
  for (let data of log) {
    if (data.qipai) {
      for (let l = 0; l < 4; l++) {
        set_qipai(data.qipai.shoupai[l]);
      }
    ...
    else if (data.hule && data.hule.fubaopai) fubaopai = data.hule.fubaopai;
  }
  ...
  for (let data of log) {
    if (data.zimo || data.gangzimo) reply.push({});
      ...
      if (data.pingju.shoupai[l]) {
        if (data.pingju.name.match(/^三家和/)) reply.push({ hule: "-" });
        else reply.push({ daopai: "-" });
      }
    } else if (data.hule) if (l == data.hule.l) reply.push({ hule: "-" });
  }
