...
module.exports = class Game {
    ...
    next(force) {
        this._timeout_id = clearTimeout(this._timeout_id);
        if (this._reply.filter(x=>x).length < 4) return;
        if (this._stop && ! force) return this._stop();
    }
    ...
    hule() {
        ...
        if (this._rule['連荘方式'] < 3 && menfeng == 0) this._lianzhuang = true;
    ...
    pingju(name, shoupai = ['','','','']) {
        ...
        if (! name) {
            ...
            let n_tingpai = 0;
            for (let l = 0; l < 4; l++) {
                if (this._rule['ノーテン宣言あり'] && ! shoupai[l]) continue;
            ...
            if (this._rule['連荘方式'] == 0) this._lianzhuang = true;
        }
        else {
            this._no_game    = true;
            this._lianzhuang = true;
