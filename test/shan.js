...
suite("Majiang.Shan", () => {
  ...
  suite("static zhenbaopai(p)", () => {
    ...
    test("東 → 南", () => assert.equal(Majiang.Shan.zhenbaopai("z1"), "z2"));
    test("北 → 東", () => assert.equal(Majiang.Shan.zhenbaopai("z4"), "z1"));
    test("白 → 發", () => assert.equal(Majiang.Shan.zhenbaopai("z5"), "z6"));
    test("中 → 白", () => assert.equal(Majiang.Shan.zhenbaopai("z7"), "z5"));
    test("不正な牌 → エラー", () =>
  ...
  suite("constructor(rule)", () => {
    test("赤牌なしでインスタンスが生成できること", () => {
      ...
      assert.equal(
        new Majiang.Shan({ 赤牌: { m: 0, p: 0, s: 0 } })._pai
          .concat()
          .sort()
          .join(),
        pai
    ...
    test("赤牌ありでインスタンスが生成できること", () => {
      const pai =
        ...
        "z5,z5,z5,z5,z6,z6,z6,z6,z7,z7,z7,z7";
      assert.equal(
        new Majiang.Shan({ 赤牌: { m: 1, p: 2, s: 3 } })._pai
          .concat()
          .sort()
          .join(),
        pai
  ...
  suite("get paishu()", () => {
    test("牌山生成直後の残牌数は122", () => assert.equal(Shan().paishu, 122));
  });
  ...
  suite("get fubaopai()", () => {
    test("牌山生成直後は null を返す", () => assert.ifError(Shan().fubaopai));
    test("牌山固定後は裏ドラを返す", () =>
