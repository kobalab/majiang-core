...
suite("Majiang.Game", () => {
  ...
  suite("qipai(shan)", () => {
    ...
    test("配牌されること", () => {
      for (let l = 0; l < 4; l++) {
        assert.equal(
          game.model.shoupai[l].toString().replace(/[mpsz]/g, "").length,
          13
        );
      }
    });
  ...
  suite("gangzimo()", () => {
    ...
    test("表示処理が呼び出されること", () =>
      assert.deepEqual(game._view._param, { update: last_paipu(game, -1) }));
    test("通知が伝わること", () => {
      for (let l = 0; l < 4; l++) {
        let id = game.model.player_id[l];
    ...
    test("カンドラ後乗せではない場合、加槓も即座に開槓されること", () => {
      const game = init_game({
        rule: Majiang.rule({ カンドラ後乗せ: false }),
  ...
  suite("hule()", () => {
    ...
    test("ダブル立直", () => {
      ...
      game.hule();
      assert.ok(
        last_paipu(game).hule.hupai.find((h) => h.name == "ダブル立直")
      );
    });
    test("槍槓", () => {
      const game = init_game({
        shoupai: ["_________m1,m111=", "_", "m23p456s789z11222", ""],
      });
      game.zimo();
      game.gang("m111=1");
      game._hule = [2];
    ...
    test("嶺上開花", () => {
      ...
      game.gangzimo();
      game.hule();
      assert.ok(last_paipu(game).hule.hupai.find((h) => h.name == "嶺上開花"));
    });
    test("最終牌で嶺上開花", () => {
      const game = init_game({
        shoupai: ["m123p456s78z11,m111=", "", "", ""],
      ...
      game.gangzimo();
      game.hule();
      assert.ok(!last_paipu(game).hule.hupai.find((h) => h.name == "海底摸月"));
    });
    test("海底摸月", () => {
      const game = init_game({
        shoupai: ["m123p456s789z1122", "", "", ""],
    ...
    test("河底撈魚", () => {
      ...
      game._hule = [2];
      game.hule();
      assert.ok(last_paipu(game).hule.hupai.find((h) => h.name == "河底撈魚"));
    });
    test("天和", () => {
      const game = init_game({
        shoupai: ["m123p456s789z1122", "", "", ""],
    ...
    test("ダブロンは親の和了があれば連荘", () => {
      const game = init_game({
        shoupai: ["m23p456s789z11122", "_", "m23p789s546z33344", ""],
        zimo: ["m2", "m1"],
      });
      game.zimo();
      game.dapai("m2");
  ...
  suite("pingju(name, shoupai)", () => {
    ...
    test("全員テンパイ", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false }),
        shoupai: [
          "m22p12366s406789",
      ...
      assert.equal(last_paipu(game).pingju.shoupai.filter((s) => s).length, 4);
    ...
    test("全員ノーテン", () => {
      ...
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, "荒牌平局");
      assert.equal(last_paipu(game).pingju.shoupai.filter((s) => s).length, 0);
      assert.deepEqual(game._fenpei, [0, 0, 0, 0]);
    });
    test("2人テンパイ", () => {
      const game = init_game({
      ...
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, "荒牌平局");
      assert.equal(last_paipu(game).pingju.shoupai.filter((s) => s).length, 2);
    ...
    test("形式テンパイとならない牌姿", () => {
      ...
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, "荒牌平局");
      assert.equal(last_paipu(game).pingju.shoupai.filter((s) => s).length, 1);
      assert.deepEqual(game._fenpei, [-1000, -1000, -1000, 3000]);
    });
    test("ノーテン宣言ありの場合、宣言なしをノーテンとみなすこと", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, ノーテン宣言あり: true }),
        shoupai: [
          "m22p12366s406789",
          "m55p40s123,z111-,p678-",
          "m67p678s22,s56-7,p444-",
      ...
      game.pingju("", ["", "_", "_", "_"]);
      assert.equal(last_paipu(game).pingju.name, "荒牌平局");
      assert.equal(last_paipu(game).pingju.shoupai.filter((s) => s).length, 3);
    ...
    test("ノーテン罰なし", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, ノーテン罰あり: false }),
        shoupai: [
          "m22p12366s406789",
          "m99p12306z277,m345-",
          "m3p1234689z55,s7-89",
    ...
    test("和了連荘の場合、親のテンパイでも輪荘すること", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, 連荘方式: 1 }),
        shoupai: [
          "m22p12366s406789",
          "m99p12306z277,m345-",
          "m3p1234689z55,s7-89",
    ...
    test("ノーテン連荘の場合、親がノーテンでも連荘すること", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, 連荘方式: 3 }),
        shoupai: [
          "m40789p4667s8z577",
          "m99p12306z277,m345-",
          "m3p1234689z55,s7-89",
    ...
    test("一局戦の場合、親がノーテンでも連荘すること", () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, 場数: 0 }),
        shoupai: [
          "m40789p4667s8z577",
          "m99p12306z277,m345-",
          "m3p1234689z55,s7-89",
  ...
  suite("last()", () => {
    ...
    test("オーラス止めなし", () => {
      const game = init_game({
        rule: Majiang.rule({ オーラス止めあり: false }),
        defen: [40000, 30000, 20000, 10000],
      });
      game.model.zhuangfeng = 1;
      game.model.jushu = 3;
  ...
  suite("jieju()", () => {
    ...
    test("1000点未満のポイントは四捨五入する", () => {
      const game = init_game({
        qijia: 0,
        defen: [20400, 28500, 20500, 30600],
        rule: Majiang.rule({ 順位点: ["20", "10", "-10", "-20"] }),
      });
      game.jieju();
    ...
    test("順位点を変更できること", () => {
      const game = init_game({
        rule: Majiang.rule({ 順位点: ["30", "10", "-10", "-30"] }),
        qijia: 2,
      });
      game.jieju();
      assert.deepEqual(game._paipu.rank, [3, 4, 1, 2]);
  ...
  suite("reply_dapai()", () => {
    ...
    test("リーチ不成立", () => {
      const game = init_game({
        shoupai: ["m55688p234567s06", "m23446p45688s345", "", ""],
        qijia: 0,
        zimo: ["s7"],
      });
      game.zimo();
    ...
    test("ポンとチーの競合はポンを優先", () => {
      const game = init_game({
        shoupai: ["_", "m23567p456s889z11", "m11789p123s11289", ""],
      });
      game.zimo();
  ...
  suite("reply_gang()", () => {
    ...
    test("ロン和了(槍槓)", () => {
      const game = init_game({
        shoupai: ["m45p456s11789,m111+", "", "", "m23456p123s67899"],
        zimo: ["m1"],
      });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: "-" }]);
    ...
    test("ロン和了(不正応答)", () => {
      const game = init_game({
        shoupai: ["m45p456s11789,m111+", "", "", "m33456p123s67899"],
        zimo: ["m1"],
      });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: "-" }]);
    ...
    test("暗槓は槍槓できない", () => {
      const game = init_game({
        shoupai: ["m11145p456s11789", "", "", "m23456p123s67899"],
        zimo: ["m1"],
      });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: "-" }]);
  ...
  suite("reply_pingju()", () => {
    test("流局", () => {
      const game = init_game({
        shoupai: ["m123p456s789z1122", "_", "_", "_"],
        qijia: 0,
        changbang: 1,
        lizhibang: 1,
        defen: [25000, 25000, 25000, 24000],
  ...
  suite("allow_hule(l)", () => {
    ...
    test("槍槓", () => {
      const game = init_game({
        shoupai: ["m1234p567s789,m111=", "", "m23p123567s12377", ""],
      });
      game.zimo();
      game.gang("m111=1");
    ...
    test("クイタンなしにできること", () => {
      const game = init_game({
        rule: Majiang.rule({ クイタンあり: false }),
  ...
  suite("static get_dapai(rule, shoupai)", () => {
    let shoupai = Majiang.Shoupai.fromString("m5678p567,z111=,s789-").fulou(
      "m0-67"
    );
    test("喰い替えなし", () =>
      assert.deepEqual(
        Majiang.Game.get_dapai(
          Majiang.rule({ 喰い替え許可レベル: 0 }),
          shoupai
        ),
        ["p5", "p6", "p7"]
      ));
  ...
  suite("static get_chi_mianzi(rule, shoupai, p, paishu)", () => {
    ...
    test("現物以外の喰い替えあり", () => {
      ...
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai1, "m1-", 1), [
        "m1-23",
      ]);
      assert.deepEqual(
        Majiang.Game.get_chi_mianzi(rule, shoupai2, "m1-", 1),
    ...
    test("現物喰い替えもあり", () => {
      ...
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai2, "m1-", 1), [
        "m1-23",
      ]);
    });
    test("ハイテイは鳴けない", () => {
  ...
  suite("static get_gang_mianzi(rule, shoupai, p, paishu)", () => {
    ...
    test("リーチ後の牌姿の変わる暗槓なし", () => {
      ...
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai1, null, 1), [
        "z1111",
      ]);
      assert.deepEqual(
        Majiang.Game.get_gang_mianzi(rule, shoupai2, null, 1),
    ...
    test("リーチ後の待ちの変わる暗槓なし", () => {
      ...
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai3, null, 1), [
        "s3333",
      ]);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai4, null, 1), [
        "s1111",
      ]);
      assert.deepEqual(
        Majiang.Game.get_gang_mianzi(rule, shoupai5, null, 1),
  ...
  suite("static allow_lizhi(rule, shoupai, p, paishu, defen)", () => {
    ...
    test("打牌が指定されていない場合、リーチ可能な打牌一覧を返す", () => {
      ...
      assert.deepEqual(Majiang.Game.allow_lizhi(rule, shoupai), ["s7", "s8"]);
  ...
  suite(
    ...
    () => {
      ...
      test("ツモ和了", () => {
        ...
        assert.ok(
          Majiang.Game.allow_hule(rule, shoupai, null, 0, 1, false, false)
        );
      });
      test("ロン和了", () => {
        let shoupai = Majiang.Shoupai.fromString("m123p456z1122,s789-");
        assert.ok(
          Majiang.Game.allow_hule(rule, shoupai, "z1=", 0, 1, false, true)
        );
      });
    }
  );
  ...
  suite("static allow_pingju(rule, shoupai, diyizimo)", () => {
    ...
    test("途中流局なし場合、九種九牌とならない", () => {
      let shoupai = Majiang.Shoupai.fromString("m1234569z1234567");
      assert.ok(
        !Majiang.Game.allow_pingju(
          Majiang.rule({ 途中流局あり: false }),
          shoupai,
          true
        )
      );
