...
suite("Majiang.Board", () => {
  ...
  suite("constructor(kaiju)", () => {
    ...
    test("起家が設定されること", () => assert.equal(board.qijia, 1));
  ...
  suite("qipai(qipai)", () => {
    ...
    board.qipai(qipai);
    test("場風が設定されること", () => assert.equal(board.zhuangfeng, 1));
    ...
    test("手牌が設定されること", () =>
      assert.equal(board.shoupai[1].toString(), "m123p456s789z1234"));
    test("捨て牌が初期化されること", () =>
      assert.equal(
        board.he.map((he) => he._pai.length).reduce((x, y) => x + y),
        0
      ));
  ...
  suite("zimo(zimo)", () => {
    ...
    board.zimo({ l: 0, p: "m1" });
    test("手番が移動すること", () => assert.equal(board.lunban, 0));
    test("牌数が減ること", () => assert.equal(board.shan.paishu, 69));
    test("手牌にツモ牌が加えられること", () =>
  ...
  suite("fulou(fulou)", () => {
    ...
    test("河から副露牌が拾われること", () =>
      assert.equal(board.he[0]._pai[0], "m1_="));
    test("手番が移動すること", () => assert.equal(board.lunban, 2));
    test("手牌が副露されること", () =>
  ...
  suite("kaigang(kaigang)", () => {
    ...
    test("ドラが増えること", () => assert.equal(board.shan.baopai[1], "s9"));
  });
